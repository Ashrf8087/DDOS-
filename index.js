...
const token = '6675791529:AAGZnTqOKTB_LHx6NABZK07AEcS3DcJwBFc';
...
function logActivity(msg) {
  const user = msg.from;
  const chat = @KING_MODS_owner;
  const command = msg.text.toLowerCase();

  console.log(`Aktivitas Penggunaan Bot Telegram`);
  console.log(`• User ID: ${user.id}`);
  console.log(`• Username: ${user.username || '@KING_MODS_owner'}`);
